...
class Program
{
    static void Main(string[] args)
    {
        ...
        var expectedDiagnostics = new[] { new DiagnosticResult(DiagnosticDescriptors.DetectMismatchedParameterOptionality).WithArguments("name").WithSpan(10, 48, 10, 59) };
...
app.MapGet(""/hello/{Age?}"", ([FromRoute] int? age) => $""Age: {age}"");
        ...
        var expectedDiagnostics = new[] { new DiagnosticResult(DiagnosticDescriptors.DetectMismatchedParameterOptionality).WithArguments("age").WithSpan(10, 30, 10, 49) };
